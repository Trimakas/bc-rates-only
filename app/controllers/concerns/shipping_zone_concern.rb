module ShippingZoneConcern
  ...
  def parse_zones(zones)
    ...
    zones.each do |zone|
      ...
      store_id = store.id
      save_or_update_zones_in_concern(name, bc_zone_id, store_id)
    end
  ...
  def save_or_update_zones_in_concern(name, bc_zone_id, store_id)
    zone_exists = Zone.where(bc_zone_id: bc_zone_id, store_id: store_id)
    if zone_exists.empty?
      Zone.create(zone_name: name, bc_zone_id: bc_zone_id, store_id: store_id)
    else
      zone_exists.update(zone_name: name)
    end
  end
